...
if __name__ == '__main__':
    model = bagnet8(pretrained=True)
    model.eval()
    while True:
        ret,frame = cap.read()
        image_name = Image.fromarray(frame)
        input_img = V(centre_crop(image_name).unsqueeze(0))
        ...
        print('{:.3f}, {}'.format(probs[0], labels['{}'.format(idx[0])]))
        #output_prob = '{:.3f}'.format(probs[0])
        #output_label = '{}'.format(str(idx2label([idx[0]])))
        #print(output_prob)
        #print(output_label)
